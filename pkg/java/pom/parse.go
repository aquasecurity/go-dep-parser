...
func (p *parser) parseRoot(root artifact) ([]types.Library, []types.Dependency, error) {
	...
	for name, art := range uniqArtifacts {
		lib := types.Library{
			...
			Licenses:  types.LicensesFromStringSlice(art.Licenses, types.NameLicenseType), // Maven recommends using SPDX licenses - https://maven.apache.org/pom.html#licenses.
			Indirect:  !art.Direct,
			Locations: art.Locations,
		}
